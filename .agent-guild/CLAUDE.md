...
2. The worker returns with the task at `needs-check`. Set it to `checking` (`task-status.py T-NNN checking`) and dispatch its checker. Where the task cites a judgment clause, send the checker of record and `checker-courier` out together in the same message: they judge the same artifact independently and neither reads the other, so running them in sequence buys nothing but wall clock.
...
Two gates hold you to this; it is not yours to remember. Every verdict of record on disk owes a crossing, one per retry round, so `-r0` and `-r1` are separate obligations. `stop-gate.py` counts the outstanding ones before it looks at open tasks at all, which means a task you already moved to `complete` still holds your turn open, and the block message names the file that's missing and the dispatch that writes it.

What holds your turn is a crossing nobody has *started*. Once `dispatch-guard` reserves one, the debt stops holding (#124): the standard-stem verdict has already decided `complete` or rework, a second opinion cannot overturn it, and making a whole wave wait on a lane that can't change the answer buys nothing. A reservation that goes stale (`AGENT_GUILD_CROSSING_STALE_S`, an hour by default) is held again, and its block message says to re-dispatch, or to waive if the dispatch can't succeed at all. The cost is that a crossing can now reach Phase 3 still open, so the retrospective records every started-and-unlanded stem by name. Unrecorded, a missing crossing reads like one that agreed. A task still sitting at `checking` with only its crossing outstanding is told to dispatch `checker-courier` before completing, in place of the generic "act on the verdict". `dispatch-guard.py` is the other half: it lets `checker-courier` through on a task carrying a debt whatever that task's status, so a demand the stop gate makes is one you can meet. That widening is courier-only—`checker-deterministic` and `checker-judgment` still need `status: checking`—and every other courier condition stands: no model override, no `workspace-write`, no dispatch into an exhausted lane.
...
- `.agent-guild/state/STALLED.md`—the stop gate writes this when one task (or a held courier debt) blocks it three times running with nothing else touching it, naming only the entities that actually tripped. Being named parks that entity: it stops holding your turn open, and the gate keeps blocking for everything else, so a job with one stuck task still gets driven. It means the loop is stuck on what it names: a checker owes a verdict, a dispute needs a ruling, or a task should be abandoned. Resolve by hand and delete it.

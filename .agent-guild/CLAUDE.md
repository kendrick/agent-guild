...
- **`…-r<N>-<lane>.denied`**, your waiver, for a host that refused the dispatch outright, and for a crossing the gate never recorded as authorized—a lane verdict can be sitting on disk and still owe, because what discharges the debt is the gate's record of the dispatch, not the file at the path. Re-dispatching won't clear it: a stem that already carries a file is never reserved, which is what stops a forged verdict being laundered by the next legitimate courier. So waive it, and name in the file why the crossing was never authorized. Also in the state map, including the lane trap.

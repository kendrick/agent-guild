...
- `.agent-guild/state/STALLED.md`—the stop gate writes this when one task blocks it three times running with nothing else touching it, naming only the tasks that actually tripped. Being named parks that task: it stops holding your turn open, and the gate keeps blocking for everything else, so a job with one stuck task still gets driven. It means the loop is stuck on what it names: a checker owes a verdict, a dispute needs a ruling, a task should be abandoned. Resolve by hand and delete it.
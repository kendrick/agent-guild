...
**Phase 1, decompose.** Invoke the `decompose` skill to turn the spec plus constitution into task files under `.agent-guild/state/tasks/`, each with an executor, a checker, and a `check_method` that cites constitution clauses. Then dispatch the auditor with `Audit-ID: DEC-audit`, which has two jobs: confirm the decomposition covers the spec, and re-read the constitution now that there is a schedule to read it against. Until a DEC-audit PASS verdict exists, `dispatch-guard` blocks every worker, and the latest round is the one that counts. There is no content fingerprint here, only in Phase 0: task files change status all job long, so a digest over them would go stale on the first transition. Coverage is the one property nothing downstream can recover: a checker verifies the task it was given, so a spec section no task covers is never built and every verdict in the job stays green while it happens.
...
- `.agent-guild/state/apparatus/<Audit-ID>-r<N>/`—scratch a CON-auditor builds in when a clause's check has to be run rather than read: a reference implementation, a throwaway repo, whatever the checks read. A venue an auditor acts *on*—a throwaway repo, a whole-tree copy—goes in its own `mktemp -d` instead, because gitignored is not isolated and a project's tooling walks its tree without consulting git (`kendrick/dotfiles#22` lost a whole repo's template rendering to one fixture left here). Round-scoped, and that is a finding source rather than housekeeping: two rounds rebuilding the same harness and disagreeing is how an ambiguous clause surfaces, which on `kendrick/dotfiles#19` happened twice and both were blockers. Gitignored and outside the job's diff scope, so none of it ships. Safe to delete whenever you want the space back, and deleted at teardown rather than moved into `archive/`, which is tracked.

...
The third job is why a DEC FAIL can now arrive keyed either way: a fork the auditor runs down gets filed against whichever document actually repairs it, `T-NNN` when a sentence in the excerpt settles it and `C-N` when only a clause revision will. Read the key, because it is telling you whether to re-cut a task or go back to Phase 0.

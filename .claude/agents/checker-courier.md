...
2. **Compose the brief.** Run `python3 .agent-guild/scripts/compose-brief.py <Task-ID> --out <scratch path>`. Build the far-side prompt from that brief plus the artifact contents inline — everything the vendor needs, in the prompt, because the vendor can read nothing else. Instruct the vendor to evaluate each cited clause against the evidence you've handed it and produce a verdict JSON: findings with concrete `evidence`, a fail needing at least one finding, `duration_ms`/`cost_usd` null (the vendor doesn't know your wall-clock time or your cost — you fill those from its usage report afterward, in the ledger, not in the verdict it hands back).

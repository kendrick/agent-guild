...
- Codex project agents are generated from `guild-core/roles/` plus `scripts/plugin-src/adapters/codex.json`. Its initializer owns only the packaged roster under the target project's `.codex/agents/` and the bounded `<!-- agent-guild:codex:start -->`…`<!-- agent-guild:codex:end -->` section in `AGENTS.md`; it never owns project config, unrelated agents, or global Codex paths.

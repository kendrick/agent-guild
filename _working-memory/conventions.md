...
- Codex project agents are generated from `guild-core/roles/` plus `scripts/plugin-src/adapters/codex.json`. The installer owns only the packaged roster under the target project's `.codex/agents/`, the packaged `.agents/skills/` files in repo-local mode, and the bounded `<!-- agent-guild:codex:start -->`…`<!-- agent-guild:codex:end -->` section in `AGENTS.md`; it never owns project config, unrelated agents or skills, runtime audition results, or global Codex paths.

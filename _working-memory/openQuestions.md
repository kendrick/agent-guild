...
## Does cross-family checking actually pay? — answered no, for one model

Settled 2026-08-13 and moved to [[decisionLog]]. #34 closed against 69 crossings across agent-guild, skills, and dotfiles: 2 real defects from the far family in 50 comparable crossings, against 102 findings both sides reached independently. The measure this section spent a week worrying about turned out to be the right thing to worry about. Of 46 findings only the in-family checker raised, 45 came from executing something or reading a repo path the brief never carried, so the rate was reporting tool access rather than model family. A courier is blind by design.

What that ruling does not cover, and what a future experiment would have to establish separately: every one of the 85 ledger rows records `model: gpt-5.6-terra`. This is one non-Claude model, not cross-family checking in general. #167 kept the lane's plumbing wired for exactly that reason, and the two data points worth carrying into any such experiment are that both real defects landed on judgment clauses, and that the corpus's single `changed_verdict: yes` came from a crossing that blocked rather than one that returned a judgment.

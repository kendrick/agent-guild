...
- `dist/codex-plugin/` — the ignored, release-stage Codex artifact generated from the shared core. Its project template carries the complete `.codex/agents/` roster and a bounded project-local initializer; only the compact `scripts/plugin-src/codex.sha256` drift lock is committed until #53 defines the publishing surface. Later v0.5.1 issues add lifecycle enforcement and complete workflow adaptation.

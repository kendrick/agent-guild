...
3. **Rebuild**: `python3 scripts/build-plugin.py`. This renders the dogfooded Claude wrappers, both published package trees, and both marketplace files from the authored release metadata.
4. **Verify**: `python3 scripts/build-plugin.py --check`. This rejects dogfood edits outside `guild-core/`, stale or manually diverged host packages, stale marketplace files, and a missing changelog section; it also runs `claude plugin validate --strict`. It has to exit 0 before anything ships.
5. **Commit** the bumped source, generated changelog section, both rebuilt package trees, and both generated marketplace files together, and nothing else. The changelog was generated before this commit existed, so the release commit itself never shows up in its own section—that's expected, not a bug to chase.

...
The one who audits the boss. Holds the orchestrator's own constitution and task breakdown to the same bar the workers face, on the principle that the orchestrator outranks workers but not the constitution. Rejects any clause that isn't falsifiable: if it can't name an artifact that would fail the clause, the clause fails. It doesn't take a check's word for what it does, either. It builds a reference implementation of the deliverable, runs every clause whose check is a command against it, expects green, then breaks it on purpose to find out whether the check notices, because a harness green in both directions verifies nothing. It sits twice, once on the constitution and once on the decomposition, and both verdicts gate: no worker is dispatched until each has passed, which makes a rubber stamp here the most dangerous move in the building. Like the checkers, it has no Edit tool: it reports, and the orchestrator revises and resubmits.

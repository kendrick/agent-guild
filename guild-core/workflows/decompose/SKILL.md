...
- `deps`: task ids whose artifacts this task needs before it can start. A dep no longer has to be `complete` for that: its worker returning is enough, so the edge costs a dispatch rather than a full check-and-verdict cycle (#135). Declare the edge on what the work genuinely needs, exactly as before; what changed is the price.
- `dep_rationale`: one line per id in `deps`, naming what this task actually needs from that task—not a summary of what the other task does in general. Fill it in whenever `deps` is non-empty; every dep edge still keeps two tasks out of one wave, so an edge nobody can justify is wall clock nobody agreed to pay. `check-job-spec.py`'s R14 checks only that the two lists correspond one to one, and only on a task that also declares `owns`—whether a rationale actually holds up is the auditor's job, not R14's.
...
Dispatch the **auditor** with `Audit-ID: DEC-audit`. Dispatch it rather than asking whether to; nothing here is the user's call, and Phase 1 carries no equivalent of Phase 0's clause-ceiling exception. One thing does cross the phase boundary: a `**Lint exception**` line waiving a heuristic lives in the constitution's preamble even when the rule fired on a task file, so a DEC-audit blocked by R2, R9, R10 or R12' is fixed in a Phase 0 artifact. The auditor confirms the tasks cover the spec, the assignments follow the routing table, and `deps` form a DAG, then re-reads the constitution against the schedule those tasks now make visible and runs every clause whose check is a command. A clause defect that only becomes falsifiable there comes back keyed to its clause id rather than to the task that surfaced it, and the fix is a clause revision and another CON round rather than a re-cut decomposition. Until that verdict passes, `dispatch-guard` blocks every worker, and a later FAIL closes the gate an earlier PASS opened. Fix what it flags and re-submit.

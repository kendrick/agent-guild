...
That PASS covers the constitution's bytes. `subagent-return` records a digest of what the auditor judged, so revising the document afterward re-closes the gate until a fresh PASS lands on the current text (#110). Only the latest round counts, here and in Phase 1: a newer FAIL closes a gate an older PASS opened.
...
- `.agent-guild/state/verdicts/CON-audit-r<N>.md.sha256`—the constitution digest that round approved, written by `subagent-return` when the auditor returns. It is what binds a PASS to specific text. Deleting one closes the gate rather than opening it, and a PASS written before this existed reads the same way: fail closed, since nothing distinguishes the two. One re-audit clears it. DEC-audit carries no stamp, because task files change status all job long and a digest over `tasks/` would go stale on the first transition.

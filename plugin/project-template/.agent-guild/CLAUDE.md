...
**Phase 1, decompose.** Run `/agent-guild:decompose` to turn the spec plus constitution into task files under `.agent-guild/state/tasks/`, each with an executor, a checker, and a `check_method` that cites constitution clauses. Then dispatch the auditor with `Audit-ID: DEC-audit` to confirm the decomposition covers the spec.
...
**Phase 3, retrospective.** Run `/agent-guild:retrospective` for the report: what the checkers caught, where retries and escalations clustered, which disputes went which way.

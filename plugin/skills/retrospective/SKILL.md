...
- **Second opinions**: only for the tasks where somebody actually dispatched a courier, which since #167 is opt-in and often none. For each one, what the far side returned and whether it disagreed with the verdict of record. A crossing that was dispatched and never landed belongs here too, named by its stem: nothing else in the run will mention it, and an unrecorded absence reads later as a crossing that agreed. Skip this item entirely on a job that dispatched no courier at all.
